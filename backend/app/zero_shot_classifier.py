...
def update_gold_column(conn):
    ...
    try:
        ...
        query = "SELECT item_id, title, description FROM ebay_listings;"
        ...
        for row in rows:
            # Perform zero-shot classification
            classifier_result = classifier(row[1] + " " + row[2],  ["gold", "non-gold"])
            is_gold = classifier_result["labels"][0] == "gold"
